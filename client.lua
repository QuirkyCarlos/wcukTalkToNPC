...
Citizen.CreateThread(function()
	...
	while true do
						...
						if distance < v.interactionRange + 3 then
							...
							if hasJob then
								...
								if distance < v.interactionRange then
									...
									if not Config.UsewcukTextUI and not inMenu then
										ESX.ShowHelpNotification('~INPUT_CONTEXT~ To talk with ~g~'..name)
									end
									if IsControlJustReleased(0, Config.Key) then
										inMenu = true
										waitMore = true
										StartCam(v.coordinates, v.camOffset, v.camRotation, v.npc, v.name)
										if Config.UsewcukTextUI then
											exports['wcukTextUI']:Close()
										end
										Citizen.Wait(500)
		...
		if inZone and not hasEntered then
			if Config.UsewcukTextUI then
				exports['wcukTextUI']:Open('[E] To talk with '..name, 'darkblue', 'left') 
			end
			hasEntered = true
...
RegisterNUICallback('action', function(data, cb)
	if data.action == 'close' then
		...
		exports['wcukNotify']:Alert("BANK", "Have a nice day.", 5000, 'info')
		if Config.HideMinimap then
			DisplayRadar(true)
		end
		hasEntered = true
...
RegisterNetEvent("wcukTalk:toilet")
AddEventHandler("wcukTalk:toilet", function()
	exports['wcukNotify']:Alert("BANK", "On your right, sir.", 5000, 'info')
end)

RegisterNetEvent("wcukTalk:rob")
AddEventHandler("wcukTalk:rob", function()
	exports['wcukNotify']:Alert("BANK", "Please stop joking, sir.", 5000, 'warning')
end)

RegisterNetEvent("wcukTalk:safe")
AddEventHandler("wcukTalk:safe", function()
	exports['wcukNotify']:Alert("BANK", "You don't have a safe, sir.", 5000, 'error')
end)

RegisterNetEvent("wcukTalk:card")
AddEventHandler("wcukTalk:card", function()
	exports['wcukNotify']:Alert("BANK", "You'll have to wait for Jennifer, sir.", 5000, 'info')
end)

RegisterNetEvent("wcukTalk:lost")
AddEventHandler("wcukTalk:lost", function()
	exports['wcukNotify']:Alert("BANK", "No problem, we'll send a new one to your home.", 5000, 'success')
end)

RegisterNetEvent("wcukTalk:jennifer")
AddEventHandler("wcukTalk:jennifer", function()
	exports['wcukNotify']:Alert("BANK", "Not at the moment, she starts at 1 PM.", 5000, 'info')
end)